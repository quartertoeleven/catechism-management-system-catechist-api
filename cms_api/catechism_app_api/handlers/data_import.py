...
def __get_middle_name(first_name_from_excel):
    return (
        " ".join(first_name_from_excel.split(" ")[1:])
        if first_name_from_excel
        else None
    )
...
def __get_date_of_birth(date_of_birth_from_excel):
    return (
        datetime.strptime(date_of_birth_from_excel, "%d/%m/%Y").date()
        if date_of_birth_from_excel
        else None
    )
...
def import_unit_students_from_excel(excel_file):
    ...
    wb = xl.book
    sh = wb["Sheet1"]
    student_df = read_excel(excel_file, sheet_name="Sheet1", header=4, dtype="string")
    ...
    for student_dict in student_dict_from_excel:
        ...
        new_student = Student(
            ...
            date_of_birth=(
                __get_date_of_birth(student_dict.get("Ngày Sinh").strip())
                if student_dict.get("Ngày Sinh")
                else None
            ),
        )
